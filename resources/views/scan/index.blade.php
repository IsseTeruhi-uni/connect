            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-800">
                    <form class="mb-6" action="{{route('scan.store')}}" method="POST" enctype="multipart/form-data">
                        ...
                        @if($request->result)
                        <div class="flex flex-col mb-4">
                            <x-input-label for="result" :value="__('生徒の解答')" />
                            <textarea class="rounded resize-none",type=result name="result" id="result":cols="30" rows="8" required
                                autofocus>{{ old('result') }}</textarea>
